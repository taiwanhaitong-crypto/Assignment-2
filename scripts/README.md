...
| `evaluate_vo_accuracy.py` | Python: runs evo to compute ATE / RPE / completeness and writes `metrics.json` |
| `05_run_evaluation_in_docker.ps1` | Runs the evaluation **inside the container**; by default uses `output/CameraTrajectory.txt` (higher completeness) and, if missing, falls back to a KeyFrame trajectory such as `expA_KeyFrameTrajectory.txt` (via `-Trajectory` flag) |
| `06_copy_camera_trajectory.ps1` | After a demo run, copies `CameraTrajectory.txt` from the container to `output/` if it exists |
